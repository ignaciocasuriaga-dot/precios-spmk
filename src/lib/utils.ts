export const TARGET_BRANDS = [
  ...
  'Tía Rosa', 'Merienda Hit', 'Rapiditas',
  'Nutrabien', 'Salmas', 'Maestro Cubano',
] as const;

export type BrandName = typeof TARGET_BRANDS[number];
...
// Keep for backward compatibility with components that import BREAD_CATEGORIES
export type BreadCategory = ProductCategory;
export const BREAD_CATEGORIES = PRODUCT_CATEGORIES;
...
export const SUPERMARKETS = ['Tata', 'Disco', 'Tienda Inglesa', 'El Dorado'] as const;
...
export function calcGap(pvp: number | null, published: number): number | null {
  ...
  return Math.round(((published - pvp) / pvp) * 100);
}
